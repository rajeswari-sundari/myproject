...
import { ManageCategories, ManageControlsSearch,ChecklistJsondata,ManageControlsdata,MetricJsonControl} from './controlLandscapeClass'
// import { MetricsJonControl}  from '../metrics-definition/metricsDefinitionSearch'

@Injectable()
export class ControlLandscapeService {
...
{audit_category_id : 1, audit_category_name : 'Test3', audit_checklist_id:3,audit_checklist:'list3' }]
    ...
    getParentCategory() : Observable<any[]> {
         return this._serviceClient.get<any>(environment.APIURL + 'AccLsManageControl/GetAccLsParentCategory').pipe();
        //return of(this.parentCategoryList);
    }
    getCategory(manageControlsSearchData) : Observable<any[]> {
        return this._serviceClient.post<any>(environment.APIURL + 'AccLsManageControl/GetAccLsCategoryList',manageControlsSearchData).pipe();
        //return of(this.categoryList);
    }
    getControls(manageControlsSearchdata : ManageControlsSearch) : Observable<any[]> {
        return this._serviceClient.post<any>(environment.APIURL + 'AccLsManageControl/GetAccLsManageControlDetails',manageControlsSearchdata).pipe()
    ...
    getmanagecategoryList():Observable<any[]>{
      return this._serviceClient.get<any>(environment.APIURL+'AccLsManageControl/GetCategoryDetails').pipe();
    }
    saveManageCategory(ManageCategories) : Observable<any[]>{
      return this._serviceClient.post<any>(environment.APIURL + 'AccLsManageControl/AddUpdateManageCategoryAccLs',ManageCategories).pipe()
      }
    ...
    AddEditControls(manageControlsdata : ManageControlsdata ) : Observable<any> {
        ...
        return this._serviceClient.post<ManageControlsdata[]>(environment.APIURL +  'AccLsManageControl/AddUpdateManageControlAccLs', manageControlsdata).pipe()
        //return of(this.ControlList);
         
     }
      saveMetrics(controls : MetricJsonControl) : Observable<any[]> {
       return this._serviceClient.post<any[]>(environment.APIURL + 'AccLsManageControl/UpdateAccLsManageControlMetricDefinition',controls).pipe()
